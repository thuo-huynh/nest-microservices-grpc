...
import { PRODUCT_SERVICE_NAME } from './order.constant';
import { ProductServiceClient } from './order.interface';
...
export class OrderService  implements OnModuleInit {
    ...
    public async createOrder(data: CreateOrderRequest) : Promise<CreateOrderResponse> {
        ...
        const decreasedStockData: DecreaseStockResponse = await firstValueFrom(this.productSvc.decreaseStock({
            id: data.productId, orderId: order.id
        }))
